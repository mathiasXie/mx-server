...
import (
	...
	"github.com/mathiasXie/gin-web/applications/asr-rpc/server"
...
func main() {
	flag.Parse()
	configFile := fmt.Sprintf("%s/%s.yaml", *configPath, "asr-rpc")
	...
	s := grpc.NewServer()
	proto.RegisterASRServiceServer(s, server.NewServer())
