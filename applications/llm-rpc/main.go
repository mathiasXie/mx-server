...
import (
	...
	"github.com/mathiasXie/gin-web/applications/llm-rpc/server"
...
func main() {
	flag.Parse()
	configFile := fmt.Sprintf("%s/%s.yaml", *configPath, "llm-rpc")
	...
	s := grpc.NewServer()
	proto.RegisterLLMServiceServer(s, server.NewServer())
