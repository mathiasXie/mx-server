...
import (
	...
	"github.com/mathiasXie/gin-web/applications/function-rpc/server"
...
func main() {
	flag.Parse()
	configFile := fmt.Sprintf("%s/%s.yaml", *configPath, "function-rpc")
	...
	s := grpc.NewServer()
	proto.RegisterFunctionServiceServer(s, server.NewServer())
